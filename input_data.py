...
def get_files(file_dir, ratio):
    for file in os.listdir(file_dir + '/Fire'):
        normal.append(file_dir + '/Fire' + '/' + file)
        label_normal.append(0)
    for file in os.listdir(file_dir + '/Normal'):
        fire.append(file_dir + '/Normal' + '/' + file)
        label_fire.append(1)


    # step2：对生成的图片路径和标签List做打乱处理
    image_list = np.hstack((normal, fire, ))
    label_list = np.hstack((label_normal, label_fire))
